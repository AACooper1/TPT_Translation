...
from transformers import AutoModel, AutoConfig, MarianTokenizer, MarianMTModel
import torch
...
ckpt = torch.load("model.mdl")

model = MarianMTModel.from_pretrained(
    None, 
    config=AutoConfig.from_pretrained('Helsinki-NLP/opus-mt-mul-en'), 
...
if __name__ == "__main__":
    ...
    if "-o" not in sys.argv:
        ...
        for batch in tqdm(dataloader):
            out = model.generate(input_ids=batch["input_ids"], attention_mask=batch["attention_mask"], max_new_tokens=256)
            # preds = out.logits.argmax(dim=-1)
            # attns = out.encoder_attentions[-1][:,0,:,:]
            results.append(compute_metrics((out, batch["labels"])))
